...
- API PREPARATION: existing_code_to_replace must be a perfect substring of the file being reviewed (`{filename}`), taken directly from the Diff section above — never from the Dependency Context section. It must match the real file content exactly (no +/- diff prefix characters). For security findings rooted in unchanged surrounding context lines, those lines are still part of `{filename}` and are valid. Never quote lines from a dependency context file.
- METHOD EXISTENCE: Never flag a method call as wrong unless the class that owns it appears in the Dependency Context AND the method is absent from it. If the class definition is not provided, you have no basis for claiming the method is incorrect — omit the finding entirely. Do not guess method names from naming conventions.
- IMPORT PATH: Never flag an import statement as broken unless the package manifest (pyproject.toml, setup.py, CMakeLists.txt, package.xml) or the target module's __init__.py appears in the Dependency Context and contradicts the import path. Package namespaces vary widely; an import that looks non-standard may be perfectly valid for this project.
- SYNTAX CERTAINTY: Flag syntax errors only when you are certain beyond any doubt. Before reporting a syntax error in a multiline expression, count every opening delimiter (parenthesis, bracket, brace) on all lines of the expression against every closing delimiter. If they balance, it is NOT a syntax error — do not report it.
